...
protocol BotEngineServiceProtocol {
    func bestMove(in boaed: Board, difficulty: Difficulty) -> CellCoordinate
}
...
struct BotEngineService: BotEngineServiceProtocol {
    func bestMove(in boaed: Board, difficulty: Difficulty) -> CellCoordinate {
        switch difficulty {
        case .easy:
            randomMove(from: boaed)
        case .medium:
            mediumMove(from: boaed)
        case .hard:
            optimalMove(for: .o, in: boaed) ?? mediumMove(from: boaed)
        }
    }
}
